...
import 'package:myWM/ui/pages/main/rekening_kredit_pin.dart';
...
class RekeningKreditOTPPage extends StatefulWidget {
  final Map<String, dynamic> dataOTP;
  const RekeningKreditOTPPage({
    super.key,
    required this.dataOTP,
  });

  @override
  State<RekeningKreditOTPPage> createState() =>
      RekeningKreditOTPPageState(dataOTP);
}

class RekeningKreditOTPPageState extends State<RekeningKreditOTPPage> {
  var logger = Logger();
  RekeningKreditOTPPageState(this.dataOTP);
  final Map<String, dynamic> dataOTP;
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        title: Text(
          'Konfirmasi OTP Pengajuan Kredit',
      ...
      body: BlocConsumer<TransferBloc, TransferState>(
        listener: (context, tState) {
          // logger.i(tState);
          if(tState is KirimUlangOTPRekeningKreditLoading || tState is ValidasiOTPRekeningKreditLoading){
            showLoadingIndicator();
          }
          if(tState is KirimUlangOTPRekeningKreditFailed){
            EasyLoading.dismiss();
            showCustomSnackBar(context, tState.e);
          }
          if(tState is ValidasiOTPRekeningKreditFailed){
            EasyLoading.dismiss();
            showCustomSnackBar(context, tState.e);
            // otpController.text = '';
          }
          if(tState is KirimUlangOTPRekeningKreditSuccess){
            EasyLoading.dismiss();
            showCustomSnackBarSuccess(context, 'Kirim Ulang Kode OTP berhasil');
          }
          if(tState is ValidasiOTPRekeningKreditSuccess){
            EasyLoading.dismiss();
            // showCustomSnackBarSuccess(context, 'SUKSES');
            Navigator.push(
              context, 
              MaterialPageRoute(
                builder: ( context) => RekeningKreditPINPage(dataPIN: tState.dataResult),
              ),
            );
          }
        },
                          ...
                          CustomFilledButton2(
                            ...
                            onPressed: () {
                              if (otpController.text.length == 6) {
                                ...
                                context.read<TransferBloc>().add(ValidasiOTPRekeningKredit(dataBody));
                              } else {
  ...
  void resendCodeOTP(dataOTP) {
    ...
    setState(() {
      otpController.text = '';
      context.read<TransferBloc>().add(KirimUlangOTPRekeningKredit(dataOTP));
      secondsRemaining = 60;
      enableResend = false;
    });
  }
