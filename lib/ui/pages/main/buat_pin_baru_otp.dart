...
class BuatPinBaruOTPPage extends StatefulWidget {
  final Map<String, dynamic> dataOTP;
  const BuatPinBaruOTPPage({
    super.key,
    required this.dataOTP,
  });

  @override
  State<BuatPinBaruOTPPage> createState() =>
      _BuatPinBaruOTPPageState(dataOTP);
}

class _BuatPinBaruOTPPageState extends State<BuatPinBaruOTPPage> {
  var logger = Logger();
  _BuatPinBaruOTPPageState(this.dataOTP);
  final Map<String, dynamic> dataOTP;
    ...
    return Scaffold(
      ...
      body: BlocConsumer<UserBloc, UserState>(
        listener: (context, userState) {
          // logger.i(userState);
          if(userState is ValidasiBuatPinBaruLoading){
            showLoadingIndicator();
          }
          if(userState is ValidasiBuatPinBaruFailed){
            EasyLoading.dismiss();
            showCustomSnackBar(context, userState.e);
          }
          if(userState is ValidasiBuatPinBaruSuccess){
            EasyLoading.dismiss();
                            ...
                            CustomFilledButton2(
                              ...
                              onPressed: () {
                                if (otpController.text.length == 6) {
                                  ...
                                  context.read<UserBloc>().add(ValidasiBuatPinBaru(dataBody));
                                } else {
  ...
  void resendCodeOTP(dataOTP) {
    setState(() {
      otpController.text = '';
      context.read<UserBloc>().add(BuatPinBaru(dataOTP));
      secondsRemaining = 60;
      enableResend = false;
    });
  }
