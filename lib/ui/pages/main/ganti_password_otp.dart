...
class GantiPasswordOTPPage extends StatefulWidget {
  final Map<String, dynamic> dataOTP;
  const GantiPasswordOTPPage({
    super.key,
    required this.dataOTP,
  });

  @override
  State<GantiPasswordOTPPage> createState() =>
      _GantiPasswordOTPPageState(dataOTP);
}

class _GantiPasswordOTPPageState extends State<GantiPasswordOTPPage> {
  var logger = Logger();
  _GantiPasswordOTPPageState(this.dataOTP);
  final Map<String, dynamic> dataOTP;
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        title: Text(
          'Konfirmasi OTP Update Password',
      ...
      body: BlocConsumer<UserBloc, UserState>(
        listener: (context, userState) {
          // logger.i(userState);
          if(userState is ValidasiGantiPasswordLoading){
            showLoadingIndicator();
          }
          if(userState is ValidasiGantiPasswordFailed){
            EasyLoading.dismiss();
            showCustomSnackBar(context, userState.e);
          }
          if(userState is ValidasiGantiPasswordSuccess){
            EasyLoading.dismiss();
            showCustomSnackBarSuccess(context, 'Password berhasil diupdate, silahkan logout untuk memastikan password anda sesuai.');
                            ...
                            CustomFilledButton2(
                              ...
                              onPressed: () {
                                if (otpController.text.length == 6) {
                                  ...
                                  context.read<UserBloc>().add(ValidasiGantiPassword(dataBody));
                                } else {
  ...
  void resendCodeOTP(dataOTP) {
    setState(() {
      otpController.text = '';
      context.read<UserBloc>().add(GantiPassword(dataOTP));
      secondsRemaining = 60;
      enableResend = false;
    });
  }
